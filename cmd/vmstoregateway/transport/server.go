...
type Server struct {
	...
	// TODO(shenjun): use bucket storage
	...
	// TODO(shenjun): how to init following struct
	// struct from mimir storegateway
	// bucket storage
	bktStores *storegateway.BucketStores
}
...
func (s *Server) processVMSelectDeleteMetrics(ctx *vmselectRequestCtx) error {
	...
	// do not delete any series and return directly.
	if err := ctx.writeUint64(uint64(0)); err != nil {
		return fmt.Errorf("cannot send deletedCount=%d: %w", 0, err)
	}
	return nil
